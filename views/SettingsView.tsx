  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-2xl mx-auto">
      <div>
        ...
        <p className="text-slate-500">Konfigurasi koneksi database Google Spreadsheet.</p>
      </div>
      ...
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex items-center gap-4 mb-6">
           ...
           <div>
             <h3 className="font-bold text-slate-800 text-lg">Koneksi Google Apps Script</h3>
             <p className="text-slate-500 text-sm">Aplikasi ini menggunakan Google Sheet sebagai database.</p>
           </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
              <Link size={16} /> URL Web App Script
            </label>
            ...
            <p className="mt-2 text-xs text-slate-400">
              Pastikan deployment script diatur ke <strong>"Anyone"</strong> (Siapa saja) agar aplikasi dapat mengakses data.
            </p>
          </div>
          ...
          <div className="bg-amber-50 p-4 rounded-xl border border-amber-100 flex gap-3">
             ...
             <div className="text-xs text-amber-800 leading-relaxed">
               <strong>Perhatian:</strong> Mengubah URL ini akan memutuskan koneksi ke database saat ini. Pastikan script baru memiliki struktur data JSON yang sesuai (courses, lecturers, rooms, schedule).
             </div>
          </div>
